<script>
  ...
  $: todosRemaining = todos.filter(todo => todo.completed === false).length;
  ...
  function editTodo(todo) {
    // Capture the original title value
  ...
  function doneEditKeydown(todo, event) {
    // Think I can use event.key === 'Enter'
    if (event.key === "Enter") {
      doneEdit(todo);
    }
    if (event.key === "Escape") {
...
<div class="container">
  ...
  {#each filteredTodos as todo (todo.id)}
    <div class="todo-item">
      <div class="todo-item-left" transition:fly={{ y: 30, duration: 300 }}>
